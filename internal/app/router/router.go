...
import (
	"github.com/ozonmp/omp-bot/internal/app/commands/rating"
	"log"
...
func NewRouter(
	...
	return &Router{
		...
		// exchange
		// estate
		rating : rating.NewRatingCommander(bot),
		// security
		// cinema
		// logistic
		// product
...
func (c *Router) handleCallback(callback *tgbotapi.CallbackQuery) {
	...
	case "estate":
		break
	case "rating":
		c.rating.HandleCallback(callback, callbackPath)
	case "security":
		break
	case "cinema":
		break
...
func (c *Router) handleMessage(msg *tgbotapi.Message) {
	...
	case "estate":
		break
	case "rating":
		c.rating.HandleCommand(msg, commandPath)
	case "security":
		break
	case "cinema":
		break
